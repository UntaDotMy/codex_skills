...
| Runtime tool/permission gate checks (Claude Code allowed-tools list, local permissions, capability-dependent fallback decisions) | 40-testing-release-production-readiness-review.md + 99-source-anchors.md |
| Tool execution safety checks (Bash safety, timeouts, rate limits, and recovery evidence) | 40-testing-release-production-readiness-review.md + 99-source-anchors.md |
...
| Optional-parallel cleanup/resource hygiene checks (close extra sessions/background processes before next task) | 40-testing-release-production-readiness-review.md + 50-feedback-style-and-remediation.md |
| Low-effort exploration quorum checks (minimum two quick exploration passes plus synthesis consensus) | 50-feedback-style-and-remediation.md + 10-requirements-traceability-and-prd-review.md |
| Medium-effort dual-pass synthesis checks (exactly two medium-effort passes, min 2 max 2, with synthesis output) | 50-feedback-style-and-remediation.md + 10-requirements-traceability-and-prd-review.md |
...
| allowed-tools compliance checks for Claude Code skills (tool list present, supported tool names, no unsupported tools) | 99-source-anchors.md + 40-testing-release-production-readiness-review.md |
...
22. Validate optional-parallel cleanup/resource hygiene evidence (no leftover extra sessions/background processes before next task).
23. Validate low-effort exploration quorum evidence (minimum two quick exploration passes plus synthesis consensus) when low exploration is used.
24. Validate medium-effort dual-pass synthesis evidence (exactly two medium-effort passes, min 2 max 2, plus synthesized output) when medium analysis/context/review passes are used.
25. Validate final-reviewer role binding evidence (reviewer role used for final review, not default).
26. Validate allowed-tools compliance for Claude Code (tool list present, permissioning understood, and fallbacks are explicit when tools are unavailable).
...
37. Validate runtime tool/permission gate handling (allowed-tools, local permissions, and capability-dependent fallback decisions).
38. Validate tool timeout/retry stability handling when tools are used (timeouts, chunking, and recovery evidence).
